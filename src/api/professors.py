...
class Professor(BaseModel):
    ...
    def must_be_alphabetic(cls, v):
        if not re.fullmatch(r"[A-Za-z]+(>[-'][A-Za-z]+)*", v):
            raise ValueError(
                "Name must contain only alphabetic characters and optional hyphen/apostrophe"
            )
        return v
...
def get_all_professors():
    with db.engine.begin() as connection:
        ...
        return [
            ProfessorOut(id=row.id, first=row.first, last=row.last, email=row.email)
            for row in rows
        ]
...
def create_professor(prof_request: Professor):
    with db.engine.begin() as connection:
        ret_id = connection.execute(
            ...
            {
                "first": prof_request.first,
                "last": prof_request.last,
