...
def find_by_schedule(schedule: List[Class]):
    ...
    with db.engine.begin() as connection:
            ...
            class_id = (
                connection.execute(
                    ...
                    ),
                    {
                        ...
                        "professor_first": item.professor_first,
                        "professor_last": item.professor_last,
                        "professor_email": item.professor_email,
                    },  # Pass as tuple for SQLAlchemy IN clause
                )
                .scalars()
                .one_or_none()
            )
        ...
        if text_ids == []:
            raise HTTPException(
                status_code=404, detail="No matching textbooks not found"
            )
