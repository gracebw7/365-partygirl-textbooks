...
def upgrade() -> None:
    ...
    op.create_unique_constraint("uq_professors_email", "professors", ["email"])
    ...
    op.create_unique_constraint(
        "uq_courses_department_number", "courses", ["department", "number"]
    )
    # add uniqueness constraing to classes for combo of course_id and professor_id
    op.create_unique_constraint(
        "uq_classes_course_id_professor_id", "classes", ["course_id", "professor_id"]
    )
    # Add uniqueness constraint to textbooks for title, author, and edition
    op.create_unique_constraint(
        "uq_textbooks_title_author_edition", "textbooks", ["title", "author", "edition"]
    )
    ...
    op.create_unique_constraint(
        "uq_links_textbook_id_url", "links", ["textbook_id", "url"]
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint("uq_professors_email", "professors", type_="unique")
    ...
    op.drop_constraint("uq_courses_department_number", "courses", type_="unique")
    # remove uniqueness constraing from classes for combo of course_id and professor_id
    op.drop_constraint("uq_classes_course_id_professor_id", "classes", type_="unique")
    # Remove uniqueness constraint from textbooks for title, author, and edition
    op.drop_constraint("uq_textbooks_title_author_edition", "textbooks", type_="unique")
    # Remove uniqueness constraint from textbook_classes for textbook_id and class_id
    op.drop_constraint(
        "uq_textbook_classes_textbook_id_class_id", "textbook_classes", type_="unique"
    )
    # remove uniqueness constraing from links for tetbook_id and url combo
    op.drop_constraint("uq_links_textbook_id_url", "links", type_="unique")
    pass
